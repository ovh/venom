...
func (Executor) Run(ctx venom.TestContext, step venom.TestStep) (venom.ExecutorResult, error) {
	...
	if e.ClientType == "producer" {
		dir, _ := os.Getwd()
		err := e.produceMessages(dir)
		if err != nil {
			result.Err = err.Error()
		}
	} else if e.ClientType == "consumer" {
		var err error
		result.Messages, result.MessagesJSON, err = e.consumeMessages()
		if err != nil {
			result.Err = err.Error()
		}
	} else {
...
func (e Executor) consumeMessages() ([]Message, []interface{}, error) {
	if len(e.Topics) == 0 {
...
reading:
	for {
		...
		case <-time.After(timeout):
			executor.Infof("Timeout reached")
			break reading
		}
	}
