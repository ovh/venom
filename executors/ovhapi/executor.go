...
func (Executor) Run(ctx venom.TestContext, step venom.TestStep) (venom.ExecutorResult, error) {
	...
	var err error
	endpoint = ctx.Bag().Get("endpoint")
	if !e.NoAuth {
		applicationKey, err = ctx.Bag().GetString("applicationKey")
		if err != nil {
			return nil, err
		}
		applicationSecret, err = ctx.Bag().GetString("applicationSecret")
		if err != nil {
			return nil, err
		}
		consumerKey, err = ctx.Bag().GetString("consumerKey")
		if err != nil {
			return nil, err
		}
	}
	...
	if insecure, err := ctx.Bag().GetBool("insecureTLS"); err == nil && insecure {
	...
	err = ctx.Bag().GetComplex("headers", &contextHeader)
	if err != nil && err.Error() != venom.NotFound("headers").Error() {
		executor.Warnf("fail to read headers from context : '%s'", err)
	}
