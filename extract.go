...
func applyExtracts(e ExecutorResult, step TestStep) assertionsApplied {
	var se StepExtracts
	var errors []Failure
	...
	for key, pattern := range se.Extracts {
		...
		errs, fails := checkExtracts(transformPattern(pattern), fmt.Sprintf("%v", e[key]), e)
		if errs != nil {
...
func checkExtracts(pattern, instring string, e ExecutorResult) (*Failure, *Failure) {
